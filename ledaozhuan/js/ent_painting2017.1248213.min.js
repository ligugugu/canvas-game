!function() {
    ...
    function t() {
        this.canvas = NE("#app")[0],
        this.ctx = this.canvas.getContext("2d"),
        this.imgList = [{
            link: "./images/cover_v2.jpg",
            imgW: "750",
            imgH: "1206"
        }, {
            link: "./images/p1.jpg",
        ...
        this.h = 1206,
        this.gif_timer = null,
        this.imgs = ["./images/cover_v2.jpg", "./images/p2.png", "./images/p3.png", "./images/p4.png", "./images/p5.png", "./images/p6.png", "./images/p1.jpg", "./images/p7.jpg", "./images/p8.png","./images/sprite.png"]
    }
    document.addEventListener("touchmove", i, !1),
    document.addEventListener("touchstart", i, !1),
    /micromessenger/.test(navigator.userAgent.toLocaleLowerCase()) && NE(".share").hide(),
    ...
    t.prototype.preload = function() {
        function i() {
            t++,
            t == s.length && a(e.imgList)

        }
        for (var t = 0, e = this, a = function() {}, s = this.imgList, n = 0; n < s.length; n++)
            this.imgList[n].image = new Image,
            this.imgList[n].image.src = s[n].link,
            this.imgList[n].image.i = n,
            this.imgList[n].image.name = n,
            this.imgList[n].image.className = "item",
            this.imgList[n].image.onload = function() {
                NE(".collection").append(e.imgList[this.i].image),
                i()
            }
            ,
    ...
    t.prototype["return"] = function() {
        function i() {
            ...
            return t.index == -1 ? (t.index = 0,
            void t.showCover()) : a - e < 1e3 / t.fps ? void (t.timer = requestAnimationFrame(i)) : (e = a,
            t.radio = 1 / t.scaleReturn * t.radio,
            t.timer = requestAnimationFrame(i),
            void t.drawReturn())
        }
        var t = this;
        this.hideend();
    ...
    t.prototype.init = function() {
        ...
        this.preload().done(function() {
            i.preImg(i.imgs).done(function() {
                ...
                NE(".loading").hide(),
                NE(".cover").addClass("active").css("opacity", "1"),
                i.domList = NE(".collection .item").sort(function(i, t) {
                    return i.name - t.name
                }),
                i.img_oversize = i.domList[i.index + 1].image,
                i.img_minisize = i.domList[i.index].image,
                i.draw(),
                NE("#start").removeClass("hidden"),
                NE("#start").bind("touchstart", i.touchHandler.bind(i)),
                NE("#start").bind("touchend", i.touchendHandler.bind(i))
            })
        }),
        NE(".return").bind("touchend", i["return"].bind(i)),
        NE(".music").bind("touchend", function() {
            NE(".music").hasClass("music_close") ? (NE(".music").removeClass("music_close"),
            NE("#audio")[0].play()) : (NE(".music").addClass("music_close"),
            NE("#audio")[0].pause())
        }),
        NE(".share").bind("touchend", function() {
            window.NewsAppShare && NewsAppShare.show(),
    ...
    t.prototype.touchendHandler = function(i) {
        ...
        i.preventDefault();
        var t = this;
        NE("#start").removeClass("active"),
        t.imgList[t.index + 1] && t.imgList[t.index + 1].gif ? (t.willPause = !0,
        NE("#start").hide(),
        NE(".gif")[0].className = "gif " + t.imgList[t.index + 1].gif,
        NE(".gif").show().css("opacity", "0")) : (t.willPause = !1,
        cancelAnimationFrame(this.timer))
    }
    ,
    t.prototype.touchHandler = function(i) {
        function t() {
            ...
            if (e.index + 1 != e.imgList.length) {
                if (i - a < 1e3 / e.fps)
                    return void (e.timer = requestAnimationFrame(t));
                a = i,
                e.imgList[e.index + 1].limitMax && e.imgList[e.index + 1].limitMin && e.radio < e.imgList[e.index + 1].limitMax && e.radio > e.imgList[e.index + 1].limitMin ? e.radio = e.scaleSlow * e.radio : e.radio = e.scale * e.radio,
                e.timer = requestAnimationFrame(t),
                e.draw()
            }
        }
        i.stopPropagation(),
        ...
        cancelAnimationFrame(this.timer),
        this.willPause = !1,
        NE("#start").addClass("active"),
        NE(".gif").hide().css("opacity", "0"),
        NE(".cover").length && NE(".cover").hide();
        var e = this
          , a = (new Date).getTime();
        clearInterval(this.gif_timer),
        e.timer = requestAnimationFrame(t)
        ...
        if (this.index + 1 != this.imgList.length) {
            if (this.radio < this.imgList[this.index + 1].areaW / this.imgList[this.index + 1].imgW)
                if (this.willPause)
                    ...
                    cancelAnimationFrame(this.timer),
                    NE(".gif").css("opacity", "1"),
                    NE("#start").show();
                else if (console.log(this.radio),
                this.index++,
                this.radio = 1,
                !this.imgList[this.index + 1])
    ...
    t.prototype.drawReturn = function() {
        ...
        this.showCover(),
        void cancelAnimationFrame(this.timer)) : (this.imgNext = this.imgList[this.index + 1],
        this.imgCur = this.imgList[this.index],
        this.img_oversize = this.domList[this.index + 1],
        this.img_minisize = this.domList[this.index],
        this.drawImgOversize(this.img_oversize, this.imgNext.imgW, this.imgNext.imgH, this.imgNext.areaW, this.imgNext.areaH, this.imgNext.areaL, this.imgNext.areaT, this.radio),
        void this.drawImgMinisize(this.img_minisize, this.imgCur.imgW, this.imgCur.imgH, this.imgNext.imgW, this.imgNext.imgH, this.imgNext.areaW, this.imgNext.areaH, this.imgNext.areaL, this.imgNext.areaT, this.radio))
    }
    ,
            ...
            if (this.radio <= this.imgList[this.index + 1].areaW / this.imgList[this.index + 1].imgW) {
                ...
                e.onload = function() {
                    var s = t.sprites
                      , n = 0;
                    NE("#start").show(),
                    this.gif_timer = setInterval(function() {
                        i.ctx.drawImage(a, 0, 0, 750, 1206),
                        i.drawSprite(e, 90, 326, n, s, 560, 516),
                        n = n + 1 == 48 ? 0 : n + 1
                    }, 50)
                }
                ,
